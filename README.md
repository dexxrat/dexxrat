# 👋 Hi, I'm Bogdan Zenevich

**Java AQA Engineer | Building Test Automation with a Product Mindset**

[![Linkedin: Bogdan Zenevich](https://img.shields.io/badge/-Bogdan%20Zenevich-blue?style=flat-square&logo=Linkedin&logoColor=white&link=https://www.linkedin.com/in/bahdan-zenevich-62b6823bb/)](https://www.linkedin.com/in/bahdan-zenevich-62b6823bb/)
[![Telegram Badge](https://img.shields.io/badge/-dexxrat-26A5E4?style=flat-square&logo=telegram&logoColor=white&link=https://t.me/dexxrat)](https://t.me/dexxrat)
[![GitHub Badge](https://img.shields.io/badge/-dexxrat-181717?style=flat-square&logo=github&logoColor=white&link=https://github.com/dexxrat)](https://github.com/dexxrat)
[![Gmail Badge](https://img.shields.io/badge/-itzenevich-EA4335?style=flat-square&logo=gmail&logoColor=white&link=mailto:itzenevich@gmail.com)](mailto:itzenevich@gmail.com)

---

## 🚀 About Me

- 🎓 **BSUIR**, Software Engineering + Business Analysis (2nd year)
- 🧪 Built a **hybrid test automation framework** from scratch
- 🏀 10 years of professional sports → discipline, teamwork, stress resistance

---

## 🛠️ Tech Stack

- Java 17, SQL, Maven
- [Selenide](https://github.com/selenide/selenide), [Selenium WebDriver](https://github.com/SeleniumHQ/selenium), [REST Assured](https://github.com/rest-assured/rest-assured) ❤️
- JUnit 5, TestNG
- PostgreSQL, JDBC
- Docker, [GitHub Actions](https://github.com/features/actions), Jenkins (basics)
- [Allure Framework](https://github.com/allure-framework), Postman, Swagger
- Git, Linux, bash
- Spring Boot, React, TypeScript (for full-stack perspective)
- IntelliJ IDEA, Jira, Confluence

---

## 📌 Featured Projects

### 🧪 [Portfolio AQA Framework](https://github.com/dexxrat/portfolio-aqa)

[![CI/CD](https://img.shields.io/badge/CI%2FCD-Passing-brightgreen?style=flat&logo=github-actions&logoColor=white)](https://github.com/dexxrat/portfolio-aqa/actions)
[![Allure](https://img.shields.io/badge/📊-Live_Report-blue)](https://dexxrat.github.io/portfolio-aqa)

**Hybrid test automation framework** built from scratch:

- ✅ 20+ tests (UI + API + DB)
- ✅ Page Object Model, Selenide
- ✅ REST Assured + JSON schema validation
- ✅ JDBC + PostgreSQL in Docker
- ✅ GitHub Actions CI/CD
- ✅ Allure reports with screenshots
